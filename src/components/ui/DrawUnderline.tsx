// 手绘下划线 —— 鼠标悬停时随机"画出"一条手绘线，移开时擦除；选中项常驻。
// 改编自 Osmo「Draw Random Underline (DrawSVG)」(https://osmo.supply)，
// 用 GSAP DrawSVGPlugin 通过 stroke-dashoffset 实现"绘制"动效；颜色用本项目 accent。
import { useEffect, useRef, type ReactNode } from 'react';
import { gsap } from '@/lib/gsapSetup';
import { cn } from '@/lib/cn';

// 6 条手绘潦草下划线（viewBox 0 0 310 40，stroke 统一交给 currentColor 上色）
const SVG_VARIANTS = [
...
const svgFor = (d: string) =>
  `<svg viewBox="0 0 310 40" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" style="position:absolute;inset:0;width:100%;height:100%;overflow:visible"><path d="${d}" stroke="currentColor" stroke-width="10" stroke-linecap="round"/></svg>`;

// 模块级游标：连续悬停依次循环到下一条线，首条随机
let nextIndex = Math.floor(Math.random() * SVG_VARIANTS.length);

interface Props {
  children: ReactNode;
  active?: boolean; // 选中态：下划线常驻
  className?: string;
}

export default function DrawUnderline({ children, active, className }: Props) {
  ...
  const draw = () => {
    ...
    if (!box || enterTween.current?.isActive()) return;
    leaveTween.current?.kill();

    box.innerHTML = svgFor(SVG_VARIANTS[nextIndex]);
    nextIndex = (nextIndex + 1) % SVG_VARIANTS.length;
  ...
  const erase = () => {
    ...
    if (enterTween.current?.isActive()) enterTween.current.eventCallback('onComplete', playOut);
    else playOut();
  };

  // 选中态切换：变为选中即画出常驻线，取消选中则擦除
  useEffect(() => {
    if (active) draw();
    else erase();
  ...
  return (
    ...
    >
      ...
      <span
        ref={boxRef}
        aria-hidden
        className="pointer-events-none absolute -bottom-[0.3em] left-0 right-0 h-[0.55em] text-accent"
      />
    </span>
  );
}
